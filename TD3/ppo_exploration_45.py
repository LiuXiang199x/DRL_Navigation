...
from test_env_exploration43 import GazeboEnv
...
class GlobalPolicy(nn.Module):
    ...
    def _get_h12(self, inputs):
        ...
        h_2 = F.max_pool2d(h, kernel_size=5, stride=5)
    ...
    def act(self, inputs, rnn_hxs, prev_actions, masks, deterministic=False):
        ...
        frontier_mask = h_12[:, 6].reshape(h_12.shape[0], -1)
        action_mask = frontier_mask * (1 - free_mask)
...
def _process_maps_frontier(maps, G):  # maps: global_map, (G,G): policy output's size   # frontier_mask
    maps = F.max_pool2d(maps, kernel_size=5, stride=5)
    '''
    thresh_obstacle = 0.6
    thresh_explored = 0.6
...
def _create_global_policy_inputs(global_map, visited_states, map_xy, frontier_map):
    ...
    frontier_mask = _process_maps_frontier(frontier_map, 240)
    frontier_map2 = torch.unsqueeze(frontier_map, 1)

    h_t = torch.cat(
        [global_map, frontier_map2, agent_position_onehot], dim=1
    )  # (bs, 4, M, M)
...
if __name__ == '__main__':
    ...
    for ep_num in range(num_episodes_start, NUM_GLOBAL_UPDATES):
        ...
        for step in range(MAX_GLOBAL_STEPS):
            ...
            if dones[0]:
                global_rewards = 3 * 5000
            else:
                if global_rewards > 0:
                    # global_rewards += current_global_episode_reward.sqrt() * 2 * 10 - (global_path_length * 10).cpu()
                    global_rewards = (3 - global_path_length / current_global_episode_reward.sqrt()) * 5000

                if global_rewards <= 4 and global_path_length > 8:
                    global_rewards = torch.ones(num_envs, 1) * (-5000.0)
            ...
            frontier_map_af = F.max_pool2d(frontier_map, kernel_size=5, stride=5)
            free_map_af = F.max_pool2d(free_space_map, kernel_size=5, stride=5) # 1 means free, different with inputs of network
            # dones = torch.sum(frontier_map, dim=(1, 2)) == 0
            dones = torch.sum(frontier_map_af * free_map_af, dim=(1, 2)) == 0
            if dones[0]:
