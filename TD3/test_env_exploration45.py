...
class GazeboEnv:
    ...
    def __init__(self, launchfile, height, width, nchannels):
        ...
        self.episode_num = 43
        self.repeat_num = 5
        self.gz_Process = subprocess.Popen(["roslaunch", "-p", self.port, fullpath, "gz_world:=TD3-2.world"])  # house_world.world"
    ...
    def reset(self, sl_Process=None):
        ...
        self.gz_Process = subprocess.Popen(["roslaunch", "-p", self.port, self.fullpath, f"gz_world:=map_{self.episode_num}.world", f"x_init:={x}", f"y_init:={y}"])
        print("Gazebo Reset!")
        time.sleep(4.0)
    ...
    def move(self, sl_Process):
        ...
        sl_Process = subprocess.Popen(
            ["roslaunch", "-p", self.port, os.path.join(os.path.dirname(__file__), "assets", "test_move_base7.launch")])
