...
from test_env_exploration63 import GazeboEnv
...
class GlobalPolicy(nn.Module):
    ...
    def act(self, inputs, rnn_hxs, prev_actions, masks, deterministic=False):
        ...
        free_map = F.max_pool2d(h_12[:, 3:4], kernel_size=8, stride=8) #!!!!!!!!!! needs pooling
        free_mask = free_map.reshape(h_12.shape[0], -1)
        action_probs3 = torch.where(free_mask == 1, action_probs, torch.ones_like(action_probs) * 0) #1e-7
    ...
    def evaluate_actions(self, inputs, rnn_hxs, prev_actions, masks, action):
        ...
        value = self.critic(h_12)
        action_probs = dist.probs
        free_map = F.max_pool2d(h_12[:, 3:4], kernel_size=8, stride=8)
        free_mask = free_map.reshape(h_12.shape[0], -1)
        action_probs3 = torch.where(free_mask == 1, action_probs, torch.ones_like(action_probs) * 0) #1e-7
        dist_3 = FixedCategorical(probs=action_probs3, validate_args=False)
        action_log_probs = dist_3.log_probs(action)
...
def _create_global_policy_inputs(global_map, visited_states, map_xy, frontier_map):
    ...
    frontier_map = frontier_map * (1 - global_map[:, 0:1])
...
if __name__ == '__main__':
    ...
    # save checkpoint
    CHECKPOINT_FOLDER = os.path.join(os.path.dirname(__file__), "new_checkpoints2")
